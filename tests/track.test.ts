...
describe('createTracker (client)', () => {
  it('sends a sanitized JSON body through the injected transport', () => {
    const sent: string[] = [];
    const t = createTracker({ enabled: true, send: (b) => { sent.push(b); return true; } });
    t.track('win', { li: 4, mv: 6, par: 6, hr: 3, hd: 0, k: 'c' });
    expect(sent).toHaveLength(1);
    expect(JSON.parse(sent[0] as string)).toEqual(
      { e: 'win', k: 'c', li: 4, mv: 6, par: 6, hr: 3, hd: 0 });
  });

  it('sends nothing when disabled', () => {
    const sent: string[] = [];
    const t = createTracker({ enabled: false, send: (b) => { sent.push(b); return true; } });
    t.track('boot');
